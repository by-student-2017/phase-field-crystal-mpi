...
dy = 2.0        # space step in x dir.

dt = 0.125      # time step

bx = 1.0        # B^x in Eq.(2.7)
bl = bx-0.05    # B^l = B^x - dB
tt = 0.585      # tau * - (phi^3)/3, Eq.(2.1) and Eq.(2.2)
vv = 1.0        # nu  * (phi^4)/4,, Eq.(2.2)
...
def init_state_seed(eta):
    ...
    seed_angles = seed_angles * 3.1415926/2.0
    seed_angles[0] = 0.0
...
def main():
    ...
    exp_part = np.zeros((3, nx, ny), dtype=np.complex128)
    for i in range(nx):
        for j in range(ny):
            for n in range(3):
    ...
    for ts in range(max_iterations):
        time_step(eta, eta_k)
    ...
    plt.pcolormesh( abs(eta[0]*exp_part[0]+(eta[0]*exp_part[0]).conjugate()
                      + eta[1]*exp_part[1]+(eta[1]*exp_part[1]).conjugate()
                      + eta[2]*exp_part[2]+(eta[2]*exp_part[2]).conjugate()) ) # Eq.(2.3), rho(r)=0 (see doc)
    plt.xlim([0, nx])
    plt.ylim([0, ny])
    plt.title("time last")
    plt.rcParams["image.cmap"] = "jet"
